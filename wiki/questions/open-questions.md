---
type: question
status: active
created: 2026-05-15
updated: 2026-05-16
sources: []
tags: [questions, gaps]
---

# Open Questions

Diese Seite sammelt offene Fragen, Wissensluecken, Unsicherheiten und Themen fuer spaetere Recherche.

## Kampagnenstart

- Sind alle SC Adepten?
- Wenn nicht, welche Rolle haben Nicht-Adepten?
- Welche Elemente sind in der Gruppe vertreten?
- Kennen sich die SC vor Session 1?
- Welche Fraktion ist einem SC persoenlich bekannt?
- Hat ein SC bereits einen [[wiki/concepts/dschinn|Dschinn]] gesehen?
- Welche Fraktion gibt den ersten Auftrag?
- Wie nah liegt die Startmine an Altin, Xian oder Kalay?
- Ist [[wiki/events/der-dschinn-in-der-mine|Der Dschinn in der Mine]], [[wiki/events/beben-und-kraterfund|Beben und Kraterfund]] oder eine Kombination aus beiden der eigentliche Start?
- Wird der erste Dschinn [[wiki/entities/items/tau|Tau]] sein oder ein anderer Dschinn aus dem Krater-Start?

## Quellenbedarf

- Konkrete SC-Konzepte und Elementzuordnungen.
- Finale Safety-, Lines-/Veils- und Tonvereinbarungen aus [[wiki/sessions/session-zero|Session Zero]].
- Daggerheart-Charakteroptionen der Spieler.
- Session-1-Notizen fuer [[wiki/events/der-dschinn-in-der-mine|Der Dschinn in der Mine]].

## Lore-Fragen

- Wie viel wissen normale Menschen zu Kampagnenbeginn ueber [[wiki/concepts/psynergy|Psynergy]]?
- Wie viel weiss die Gruppe ueber die flache Welt und [[wiki/entities/places/gaia-falls|Gaia Falls]]?
- Gibt es eine dritte Option fuer [[wiki/concepts/alchemie|Alchemie]] jenseits von Siegel halten oder vollstaendig befreien?
- Sind Licht und Dunkel Teil dieser Kampagne oder spaetere Erweiterung?
- Was genau liegt unter dem Abgrund am [[wiki/entities/places/weltrand|Weltrand]]?
- Ist [[wiki/entities/places/weyard|Weyard]] natuerlich, erschaffen oder alchemisch stabilisiert?

## Regel-Fragen

- Wie viele [[wiki/concepts/dschinn|Dschinn]] darf ein SC gleichzeitig ausruesten?
- Koennen [[wiki/concepts/dschinn|Dschinn]] frei zwischen SC wechseln?
- Brauchen [[wiki/concepts/dschinn|Dschinn]] Zustimmung fuer Bindung oder Wechsel?
- Gibt es [[wiki/concepts/dschinn|Dschinn]]-basierte Klassenwechsel oder bleibt es bei passiver Experience und Entfesseln?
- Wie stark darf Heil-[[wiki/concepts/psynergy|Psynergy]] sein?
- Wie oft sollen [[wiki/concepts/beschwoerungen|Beschwoerungen]] moeglich sein?
- Sollen [[wiki/concepts/psynergy-stein|Psynergy-Steine]] eine harte Ressource haben?
- Mutieren [[wiki/concepts/psynergy-stein|Psynergy-Steine]] grundsaetzlich Tiere und Pflanzen oder nur im Krater-Start?

## Fraktionsfragen

- Welche Fraktion wird zuerst zum offenen Gegner?
- Welche Fraktion hat am meisten recht?
- Welche Fraktion hat das gefaehrlichste Geheimnis?
- Ist [[wiki/entities/places/prox|Prox]] frueh als menschlich sichtbare Seite geplant oder anfangs nur Feindbild?
- Sind die [[wiki/entities/factions/entfessler|Entfessler]] eine eigene Fraktion oder eine radikale Spieltisch-Bezeichnung fuer Teile der [[wiki/entities/factions/proxeanische-rettungsfraktion|Proxeanischen Rettungsfraktion]]?
- Sind die [[wiki/entities/factions/puritanische-waechter|Puritanischen Waechter]] eine Splittergruppe der [[wiki/entities/factions/hueter-des-siegels|Hueter des Siegels]] oder eine eigenstaendige Jenei-Tradition?

## Ortsfragen

- Welche Route fuehrt nach dem ersten Arc weiter?
- Wann soll [[wiki/entities/places/vale-und-aleph-massiv|Vale]] erstmals relevant werden?
- Wird der [[wiki/entities/places/weltrand|Weltrand]] tatsaechlich besucht?
- Ist [[wiki/entities/places/lemuria|Lemuria]] ein spaetes Kampagnenziel oder nur Mythos?
- Liegt der Krater-/Vulkanausbruch-Start auf [[wiki/entities/places/gondowan-und-lalivero|Gondowan]], auf der [[wiki/entities/places/altin-xian-kalay-route|Altin-Xian-Kalay-Route]] oder an einem anderen Ort?

## Dschinn-Fragen

- Ist [[wiki/entities/items/tau|Tau]] sicher der erste Dschinn?
- Soll jeder SC frueh einen Dschinn erhalten?
- Haben [[wiki/concepts/dschinn|Dschinn]] eigene Namen aus dem Golden-Sun-Kanon oder neue kampagnenspezifische Namen?
- Wie persoenlich sprechen [[wiki/concepts/dschinn|Dschinn]] mit SC?
- Koennen [[wiki/concepts/dschinn|Dschinn]] sterben, verletzt werden oder korrumpieren?

## Kanon-Konflikte

- Was passiert, wenn SC eine bekannte Kanonfolge verhindern wollen?
- Duerfen SC [[wiki/entities/characters/isaac|Isaac]] oder [[wiki/entities/characters/felix|Felix]] direkt beeinflussen?
- Welche Ereignisse passieren fest im Hintergrund?
- Welche Ereignisse sind offen fuer Veraenderung?
